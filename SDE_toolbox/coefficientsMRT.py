...
def getRotationDiffusionMRT(TBar, DCoefs, savepath, filename = 'theorPsiRedCoefs'):
    ...
    for i in range(nSteps1):
        ...
        V[i] = VMRT(PsiV[i], TBar, DCoefs, nSteps=1000)
    VFunc = interp1d(PsiV, V)
    
    plt.plot(PsiV, V, label = r'$V(\theta)$')
    plt.xlabel(r'$\theta$')
    plt.legend()
    plt.show()
    ...
    for i in range(nSteps2):
        ...
        IPlus[i] = IMRT(PsiI[i], VFunc, DCoefs, sign=1, nSteps=20000)
        IMinus[i] = IMRT(PsiI[i], VFunc, DCoefs, sign=-1, nSteps=20000)
        
    plt.plot(PsiI, IPlus, label = r'$I_+(\theta)$')
    plt.plot(PsiI, IMinus, label = r'$I_-(\theta)$')
    plt.xlabel(r'$\theta$')
    plt.legend()
    plt.show()
    ...
    with open(f'{savepath}/{filename}.pck', 'wb') as file_handle:
        ...
        pickle.dump(2*np.pi/TBar, file_handle)
